  ...
  <div class="col-md-12">
    ...
    <div class="card">
      <div class="card-body">
        ...
        @endif
        @csrf
        <input type="hidden" name="id" value="{{isset($order) ? $order->id : ''}} {{session('order') ? session('order')['id'] : '' }}">
        <div class="col-md-12">
          <div class="row">
            <div class="col-lg-6 col-sm-12 mb-3">
              <label>ID member</label>
              <input type="text" name="id_member" value="{{isset($order) ? $order->id_member : ''}} {{session('order') ? session('order')['id_member'] : '' }}" class="form-control" required>
            </div>
            <!-- -->
            <div class="col-lg-6 col-sm-12 mb-3">
              <label>Kode Barang</label>
              <input type="text" name="kode_barang" value="{{isset($order) ? $order->kode_barang : ''}} {{session('order') ? session('order')['kode_barang'] : '' }}" class="form-control" required>
            </div>
            <!-- -->
            <div class="col-lg-6 col-sm-12 mb-3">
              <label>jumlah</label>
